...
+ 흐름 제어 : 송신 측은 수신 측의 데이터 처리 속도를 감안해서 전송할 데이터 크기를 제어한다.
  ...
  + Sliding Window
    + 수신 측에서 설정한 윈도우 크기만큼 송신 측에서 확인 응답없이 데이터를 전송하는 방법
    + 수신측의 버퍼를 고려하여 ACK에 남은 버퍼 크기를 담아서 전송하고 송신측은 남은 버퍼 크기를 고려하여 데이터를 전송한다.
...
+ REST
  ...
  + 단점
    + 사용할 수 있는 메소드가 제한적이다.
  + 필요성
    + 다양한 플랫폼(브라우저, 모바일)에 지원을 위한 아키텍처가 필요하고 이용방법이 필요하기 때문에 REST를 선택
+ REST API
  + REST 기반으로 만들어진 API
+ REST ful
