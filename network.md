...
+ Http 통신은 TCP통신 기반.
  + Http 1.1부터 2.0까지 TCP기반으로 만들어졌고 계층만 다를 뿐 근본은 TCP.
  + 단기적으로 새로운 커넥션을 하는 TCP라고 볼수 있다.
  + TCP기반 통신이기 때문에 Http통신은 3way hand shake과정이 있다.
...
+ 세션은 별도의 저장공간을 필요로 하고 쿠키는 정보가 변질된 가능성이 있다.
...
+ REST
  ...
  + 단점
    + 사용할 수 있는 메소드가 제한적이다.
  + 필요성
    + 다양한 플랫폼(브라우저, 모바일)에 지원을 위한 아키텍처가 필요하고 명확한 이용 방법이 필요하기 때문에 REST를 선택
+ REST API
  + REST 기반으로 만들어진 API
+ REST ful
