...
5. 세션 계층 (5계층)
   ...
   - 통신 연결은 포트 기반으로 구성하여 연결하며, 통신을 위한 세션 연결/유지/종료 과정을 수행한다.
...
+ <img width="834" alt="image" src="https://user-images.githubusercontent.com/57162257/218350424-e3d6e4db-72a4-4e12-8a85-dfd686ba5313.png">
+ <img width="835" alt="image" src="https://user-images.githubusercontent.com/57162257/218350840-a277018f-a28b-44e6-9b04-8e1df867e6ba.png">
+ 물리계층(1계층) : 디지털 신호를 아날로그 신호로 변환하여 전달하는 계층, 하드웨어 기술 제공
+ 데이터 링크 계층(2계층) : 같은 네트워크 안에서 여러대의 컴퓨터들이 데이터를 주고받기 위해 필요한 모듈
  + 데이터를 구분해서 받기 위해 `000`또는 `111`과 같은 표시를 하여 그 사이에 있는 데이터를 구분하여 데이터를 인코딩, 디코딩한다. (framing)
  + Mac주소를 이용하여 직접 연결된 서로 다른 장치간 데이터 전송 담당
  + 프레임

+ 네트워크 계층(3계층) 
  + 목적지 컴퓨터에 데이터를 전송하기 위해 목적지의 주소(IP)를 설정하고 IP를 통해 목적지 까지의 길을 찾아주는 역할
  + 다른 네트워크에 있는 목적지라면 다음 라우터에게 데이터를 넘겨줌
  + 패킷
+ 전송 계층(4계층)
  + Port를 이용하여 목적지의 프로세스까지 데이터를 전달시켜주는 역할
+ 애플리케이션 계층(7계층)



- 'hello'데이터를 전달하는 과정
  1. (7계층) 송신 컴퓨터의 애플리케이션 계층에서 'hello'데이터를 작성한다.
  2. (6계층) 표현계층에서 데이터를 압축 및 변형한다.
  3. (4계층) 전송계층에서 목적지 프로세스의 port번호와 함께 data를 감싼다.
  4. (3계층) 네트워크계층에서 목적지 IP주소를 할당한다.
  5. (2계층) 데이터 링크계층에서 직접 연결된 장치의 mac주소를 설정
  6. (1계층) 인코더를 통해 데이터를 아날로그신호로 변경하여 다음 장치로 전달
  7. (1계층) 수신받은 컴퓨터에서 아날로그신호를 디지털 신호로 변경
  8. (2계층) mac주소를 통해 목적지 mac 주소인지 확인
  9. (3계층) IP주소를 통해 목적지 IP 주소인지 확인
  10. (4계층) mac과 ip주소가 동일한 데이터이므로 전송계층에서 port를 가진 프로세스에게 데이터 전달
  11. (6계층) 데이터 압축 풀기 및 변형
  12. (7계층) 프로세스에 들어온 데이터를 응용 프로그램을 통해 'hello'라는 메시지 출력
...
+ 실질적으로 자주사용하는 패킷 기반의 통신을 전달하기 위해 TCP와 IP를 함께 사용하여 전송하는 통신 규약
...
- TCP는 연결지향 프로토콜이기 때문에 논리적 연결을 수행해야한다. 그렇기 떄문에 서버와 클라이언트간의 통신을 위해서는 특별한 신호 절차를 거쳐야한다.

  - 논리적 연결을 위한 절차를 3-way handshaking이라고한다.
  - 3-way handshaking을 하기 위해 tcp의 control bit를 사용한다. (sync, ack 플래그)
...
- 4 - way handshaking
  ...
  - 세션을 종료하기 위해 수행되는 절차 (tcp control bit 사용 - fin, ack)
    ...
    + RSA : 소인수 분해 문제에 기반을 둔 알고리즘
      + PFS를 제공하지 않기 때문에 주로 사용되지 않는다
        + PFS : 비밀키가 제 3자에게 노출되면 과저의 모든 암호화된 데이터를 해독해버릴수 있기 때문에 기존 암호문을 노출된 비밀키로 해독할 수 없도록하여 과거의 데이터의 보안성 보장 해주는 방법. (세션키의 수명을 짧게 유지)
          + (?) 세션키가 제 3자에게 노출되면 해당 세션의 데이터에만 영향이 있고, 이전의 세션 데이터는 복호화가 불가능하게 하여 데이터의 보안성을 보장해주는 기능.
        + DHE단계에서 도출되는 키를 암호화 작업이 끝나면 파기시키고 필요할 때 다시 새로운 키를 만드는 특성
...
+ 쿠키와 세션 차이
  + 저장위치
    + 쿠키는 브라우저의 메모리 또는 파일에 저장되고 세션은 서버의 메모리에 저장된다.

  + 보안
    + 쿠키는 브라우저의 로컬에 저장되기 때문에 보안이 비교적으로 취약하다. 세션은 정보가 서버 내부에 저장되어있기 때문에 비교적 안전하다.

  + 라이프 사이클
    + session cookie를 사용하면 브라우저의 메모리에 저장되기 때문에 브라우저가 종료되면 세션 쿠키도 사라진다.
    + Persistence cookie를 사용하면 사용자의 파일에 저장되기 때문에 브라우저가 종료되어도 만료시간이 지날때까지 유지된다.
    + 세션은 서버에서 지정한 만료시간이 지나면 삭제되거나 세션을 세션쿠키에 저장한다면 브라우저 종료시 삭제될 수 있다.

  + 속도
    + 쿠키에 정도가 들어있기 때문에 서버에 요청시 헤더를 바로 참조하면 되기 때문에 속도가 빠르다
    + 세션id를 이용해서 서버에 데이터를 다시 참조해야하기 때문에 속도가 비교적 느리다.
...
+ JWT
  ...
  + 리프레시 토큰을 탈취 당한 경우
    + 리프레시 토큰을 통해 만료된 accessToken을 요청하게 되는데, accessToken을 요청할때, 클라이언트의 계정 정보도 함께 요청해야하기 때문에 리프레시 토큰만 탈취당했다면 accessToken 발급에 영향을 미치지 않는다.
...
+ REST
  + URI를 통해 데아터, 이미지 등의 자원을 명시하고 HTTP 메소드를 통해 자원을 처리하도록 설계된 아키텍처
  + 필요성
    + 다양한 플랫폼(브라우저, 모바일)에서 사용할 명시적인 Http 아키텍처가 필요했다.
  + 특징
    + Uniform Interface
      + URI로 명시한 자원에 대한 조작을 공통적이고 한정적인 인터페이스를 통해 수행하는 아키텍처
    + Stateless
      + 작업을 위한 상태를 별도로 저장하지 않는다.
      + 구현이 단순해진다.
    + Self-description
      + REST자체만으로 어떤것을 요구하는지 추론할수 있다
    + Caching
      + HTTP를 그대로 사용하기 때문에 추가적인 인프라를 사용하지 않는다.
      + HTTP에서 사용되는 캐싱 기능을 사용할 수 있다.
        + GET에서 HTML, 데이터를 캐싱
    + Server-Client
      + 클라이언트와 서버를 구분하여 역할이 명확해짐.
    + 계층형 구조
      + REST서버는 다중 계층으로 구성될수 있으며, 보안 로드밸랜싱, 암호화 계층을 추가해 구조상 유연성을 둘 수 있다.
  + 장점
    + 서버와 클라이언트를 구분할수 있다.
    + 거대한 애플리케이션에서 각 모듈과 기능을 구분할 수 있다.
    + 웹 브라우저 뿐만 아니라 앱에서도 기능 사용이 가능하다.
  + 단점
    + 사용할 수 있는 메소드가 제한적이다.
    + 표준규약이 없다 (안티 패턴을 판단하는 것은 암묵적 규칙에 의한 것)
+ REST API
  + URI로 자원을 명시하고, Http Method로 자원을 처리하는 인터페이스
+ REST ful
...
- GraphQL
  - Server API를 구성하기 위한 쿼리 언어.
  - SQL은 데이터베이스로 부터 데이터를 가져오는 목적, GraphQL은 클라이언트가 데이터를 서버로부터 가져오는 것을 목적
  - REST API의 단점인 over fetching과 under fetching 문제를 해결할수 있는 API 방법
    - over fetching : 필요하지 않은 데이터도 함께 받아오는것.(너무 많은 데이터)
      - 영화정보에서 이름만 불러오고 싶을때 REST API에서는 영화정보를 전부 불러와서 필요한 정보만 걸러서 사용
    - under fetching : 너무 적은 데이터
      - 상영중인 영화리스트와 개봉예정인 영화리스트를 불러올때 REST API에서는 두개의 요청을 보내야한다.
  - 특징
    -  하나의 endpoint를 가진다.
      - 하나의 endpoint에서 요청 데이터만 변경하여 유연한 결과를 받는다.
    - 요청할때 사용하는 쿼리에 따라 다른 응답을 받는다.
  - 장점
    - HTTP 요청 횟수를 줄일 수 있다.
    - HTTP 응답 사이즈를 줄일 수 있다.
  - 단점
    - 고정된 요청과 응답을 필요로 할때는 쿼리의 요청 크기가 rest api보다 커질수 있다.
...
+ 소켓
  ...
  + 소켓은 응용프로그램의 프로세스와 전송계층과의 연결을 위한 인터페이스 역할을 수행한다.
  + 소켓은 전송계층 위에서 동작하기 떄문에 TCP와 UDP를 사용한다.
