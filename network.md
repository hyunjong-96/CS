...
+ HTTPS(Hyper Text Transfer Secure)
  ...
  + HTTPS 동작과정
    ...
    1. 클라이언트가 서버에 최초 연결을 시도함
    2. 서버는 인증서(공개키)를 클라이언트에게 전달.
    3. 클라이언트는 인증서의 유효성 검사를 한 후 세션키 발급.
    4. 클라이언트는 세션키를 공개키로 암호화하여 서버에 전달.
...
+ JWT
  + JWT는 토큰을 만들어 발행하는 방식으로 별도로 저장공간이 필요없고 사용자 정보가 암호화되어 정보 변질 가능성이 없다.
  + JWT를 통해 서버에 요청하면 유효한 토큰 여부만 검증하면 된다.
  + 하지만 탈취가능성이 있기 때문에 JWT의 유효기간을 짧게 가지고 Refresh토큰을 이용해 새 토큰을 받게 한다.
...
+ 목적
  + 악용 도메인이 다른 도메인(네이버, 카카오)의 URL정보를 가져와 사용자의 웹 브라우저에 올리는 거나 서버에 악의적인 요청을 보내는것을 막기 위함.
...
+ 소켓
  + 네트워크상에서 데이터를 받기 위한 창구 역할 로써, 프로토콜(http, https), IP, 포트로 구성된다.
  + 소켓 통신
