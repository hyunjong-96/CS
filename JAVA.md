...
+ 컴파일러와 인터프리터를 병행하는 이유
  + 소스코드를 바이트코드로 컴파일함으로써, 어떤 환경(mac, window)에서도 코드의 이식성을 높이고 외부환경에 소스코드를 보호해줄수 있습니다.
  + 바이트코드를 기계어로 인터프리터함으로써, 소스코드의 문법 검사등의 불필요한 작업없이 빠르게 번역시간을 줄여줄수 있습니다.
...
+ 추상화(Abstraction)
  ...
  + 추상 클래스 : 추상메소드로 구현해야하는 것을 정의하고 일반 메소드로 기능(추상메소드 정의 기능x)을 구현하여 자식 클래스가 부모 클래스의 기능 사용과 자식클래스를 통한 구체화로 확장을 목적을 한다.
...
+ 인터페이스는 구현을 강제하여 구현 클래스의 동일한 동작 수행을 보장하고 표준화를 유도하는 목적 (설계도).
  ...
  + 인터페이스 끼리 다중상속이 가능하다
+ 추상 클래스는 추상메소드로 구현해야하는 것을 정의하고 일반 메소드로 기능(추상메소드 정의 기능x)을 구현하여 자식 클래스가 부모 클래스의 기능 사용과 자식클래스를 통한 구체화로 확장을 목적을 한다.
  + kind of : 의 종류
    + 확장성
...
+ IoC
  + 제어의 역전
  + 객체 생성, 라이프사이클 관리 등의 코드의 흐름 관리를 개발자가 아닌 프레임워크에게 위임하는것.
  + 사용자가 자판기에서 음료수를 사먹을때, 자판기가 꺼내줄 음료를 사용자가 사용에 필요한 코드를 알 필요가 없다.
    사용자가 원하는 음료를 사용자가 만드는 것이 아닌, 프레임워크가 자판기에서 사용될 음료와 자판기 내부 구조를 사용자 대신 생성하고 관리함으로써, 사용자가 자판기에 대한 의존성을 느슨하게 만들어주고 재사용성을 향상시켜준다.
+ DIP
  + 의존 역전의 원칙
  + 구체적인것에 의존하는 것이 아닌, 추상적인 것에 의존해야하는 원칙
  + DIP를 위배하고 구체적인것에 의존한다면, 다른 구체적인 것을 사용하려할떄, 의존하고 있는 객체 내부 코드는 변경되어 OCP를 위반하게된다.
  + 추상적인것에 의존함으로써, 다형성을 통해 유지보수와 재사용성을 향상시킬 수 있다.

+ DI
  + 의존성 주입
  + 의존하고 있는 객체를 내부에서 생성하는 것이 아닌, 외부에서 주입하는 패턴
  + 외부에서 주입함으로써, 의존 객체간의 결합도가 느슨해지고 재사용성 향상
...
+ IoC와 DI
  ...
  + DI는 IoC는 따름으로써, 사용처에서 주입해야하는 의존 객체와 느슨한 결합도와 재사용성을 제공한다.
...
+ 컴파일 시점에 타입을 체크함으로서 코드의 안정성을 높여주는 기술
  ...
  + 장점
    ...
    + 다른 타입을 가지고 동일한 작업을 할떄 재사용성을 높여줄수 있다.
