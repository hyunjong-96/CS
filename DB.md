...
+ 분산형 구조는 일관성(Consistency), 고가용성(Availability), 분산 허용(Partition Toleran)의 3가지 특징을 가지고 있다.
  + 일관성(C) : 어떤 노드를 접근하여도 항상 동일한 데이터를 제공받는다.
  + 고가용성(A) : 클러스터링된 노드에 문제가 발생하여도 정상적으로 처리가 가능하다.
  + 분산 허용(P) : 클러스터링된 노드간에 네트워크 오류가 발생하여도 정상적으로 서비스 수행이 가능하다.
...
+ NoSQL
  + Consistency + Partition Toleran : 모든 노드가 함께 서비스를 제공하는 성능 위주의 제공
