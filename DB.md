...
+ Cluster 인덱스
  + <img width="400" alt="image" src="https://user-images.githubusercontent.com/57162257/193433771-10ba2207-a9f2-43f0-b0ad-3d74b16de851.png">
  + 인덱스로 지정한 컬럼을 기준으로 물리적으로 저장되는 순서를 정렬하는 인덱스
  + 하나의 테이블당 1개.
  + 인덱스를 기준으로 정렬되어 저장되어있기 때문에 검색에서 효율성이 좋다.
    하지만 삽입 삭제시 정렬된 데이터를 재정렬하는 과정으로 많은 비용이 소모된다.
    + Dense Index
...
+ Non-Cluster 인덱스
  + <img width="400" alt="image" src="https://user-images.githubusercontent.com/57162257/193433778-f4173377-4dc1-47fb-a702-f0ae0fecedf3.png">
  + 인덱스값을 기준으로 정렬하여 새로운 인덱스 페이지를 만드는 인덱스
  + 테이블에 저장된 물리적인 순서가 정렬되지 않는다.
  + Root 페이지, Leaf 페이지, Date 페이지로 나뉘어지며, Leaf 페이지에서 실제 데이터가 위치한 주소를 가리킴.
  + 테이블당 여러개 가능
  + index순서대로 정렬되어 있지 않기 때문에 범위조건 사용시 검색이 느리고 leaf page에서 한번더 조회를 해야하기 때문에 검색이 느리다. 데이터 추가,삭제시 Data Page 구조 변경없이 Leaf 페이지 정보만 변경해주면 되기 때문에 ClusterIndex보다 변경이 유리하다.
