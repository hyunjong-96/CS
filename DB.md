...
+ DBMS (DataBaseManagementSystem)
  + 여러 사용자들이 DB를 사용할 수 있도록 하고, 조작하는 소프트웨어
+ 스키마
...
+ 데이터베이스 특징
  ...
  + 데이터 무결성
    + 일관성과 정확성을 유지하여 데이터의 결함이 없도록 보호하는 기법
      + 무결성 : 데이터가 정확한 것
      + 정합성 : 다른 데이터에 대해 일관성있는 데이터를 보장(이상현상이 발생하지 않는 데이터)
    + 일관성 : 데이터의 상태가 유지되는것
    + 정확성 : 데이터 중복x, 누락x
  + 데이터 보안성
...
+ 기본키와 외래키는 변경이 가능할까?
  + 기본키는 연관관계가 없는 경우 기본키 변경이 가능하다.
  + 연관관계가 있고 cascade가 없는 경우 기본키 변경은 불가능하다.
  + 연관관계가 있고 update cascade가 있는 경우 기본키 변경이 가능하다.(부모 테이블)
  + 연관관계가 있고 부모 테이블에 존재하는 키일 경우 외래키는 변경 가능하다.
...
+ 클러스터 인덱스 (Cluster Index)
  ...
  + Clustered Index Seek
    + Root페이지에서 Leaf 페이지까지 클러스터 인덱스 컬럼 탐색을 통해 원하는 데이터를 가져오는 방법
...
+ 종류
  + 규칙 기반 옵티마이저 (Plain Base Optimizer - PBO)
    + 실행 속도가 빠른순으로 우선순위를 정하고, 우선순위가 높은 실행계획을 채택하는 방법
...
- RDB (Relation Data Base)
  ...
  - 수직적 확장이 가능하다 (성능 향상)
  - 장점
    - 명확한 스키마를 가져 구조가 정해져있다.
    - 중복을 방지할 수 있다.
    - 데이터 무결성을 보장한다.
  - 단점
    - 스키마를 사전에 설정해야하고 스키마 변경이 어렵다.
    - 데이터간 관계를 맺고 있어 JOIN문으로 쿼리가 복잡해진다.
  - 사용
...
- No SQL
  ...
  - 장점
    - 수직적, 수평적 확장이 가능하다.
    - 스키마가 없어 자유롭게 데이터 저장이 가능하다.
  - 단점
    - 스키마가 없어 데이터 구조가 불명확한다.
    - 데이터 중복 저장으로 인해, 동일한 데이터 수정시 해당 데이터를 모두 찾아 수정해야한다.
  - 사용
...
+ RDB에서는 테이블간의 연관관계를 가지고 있고 테이블이 분리되어있다. 그렇기 때문에 테이블별로 샤딩을 하게되면 조회하려는 데이터와 관련된 데이터를 찾기위해 모든 테이블을 뒤져야할수도있기때문에 복잡해진다.
+ NoSQL에서는 조회하고자 하는 데이터를 찾으면 해당 도큐먼트에 관련된 데이터가 함께 저장되어있어 관련데이터와 함께 빠르게 조회할수있기때문에 RDB보다 NoSQL이 보다 수평적확장에 유리하다.
+ <img width="874" alt="image" src="https://user-images.githubusercontent.com/57162257/219303818-d79c99b0-6e04-4d76-b39a-870f203c5073.png">
  + 구간으로 나누는 기준을 이름으로 놓고, 이름과 관련된 데이터가 하나의 도큐먼트에 저장되어있기 때문에 '철수'를 조회한다면 관련된 데이터를 한번에 조회할수있기때문에 수평적확장에 유리하다
...
+ 분산형 구조는 일관성(Consistency), 고가용성(Availability), 분산 내구성(Partition Toleran)의 3가지 특징을 가지고 있다.
  + 일관성(C) : 어떤 노드를 접근하여도 항상 동일한 데이터를 제공받는다.
  + 고가용성(A) : 클러스터링된 노드에 문제가 발생하여도 정상적으로 처리가 가능하다.
  + 분산 내구성(P) : 클러스터링된 노드간에 네트워크 오류가 발생하여도 정상적으로 서비스 수행이 가능하다.

+ CAP은 이 분산형 구조의 특징 중 2개만 만족할 수 있다는 이론
+ RDBMS는 C, A (MySQL-galera)
  + 시스템이 다운되어도 데이터 손실을 방지한다.
  + 트랜잭션을 사용하여 결과를 제공한다.

+ NoSQL
  + Consistency + Partition Toleran : 모든 노드가 함께 서비스를 제공하는 성능 위주의 제공 (MongoDB, Redis)
  + Availability + Partition Toleran : 비동기화된 서비스 스토어에 적용
...
+ Redis
  ...
  + 장점
    + 다양한 자료구조 지원
      + String, LinkedList, Set, Sorted Set 등의 다양한 자료구조를 지원한다.
    + Master-Slave 구조로 데이터를 복사하여 고가용성을 제공한다.
    + 트랜잭션기능 지원
    + 스냅샷을 통해 디스크에 저장이 가능하다.
  + 단점
    + 싱글스레드이기 때문에 한번에 1개의 명령만 수행이 가능하다.
      + 모든 데이터 읽기, 삭제는 지양
+ Memcached
  + In Memory 데이터베이스로 key,value 형태로 데이터를 저장하는 NoSQL이다.
  + 장점
    + 멀티스레드를 지원함으로써 한번에 많은 요청을 처리할 수 있다.
  + 단점
    + 복사, 트랜잭션, 스냅샷 기능을 지원하지 않는다.
+ In Memory 데이터베이스란
...
+ 테이블 내부에 많은 데이터가 있어 검색과 저장공간의 비효율성을 개선과 데이터베이스 트래픽 분산을 목적으로 사용
+ 장점
...
+ 파티셔닝
  ...
  + 하나의 테이블의 크기가 커지면 시스템 용량의 한계와 성능 저하 문제가 발생하기 때문에 테이블을 분리함으로써 이러한 문제를 해결하기 위한 방법
  + 장점
...
+ 동일한 DB 스토리지를 이용하고 다수의 DB 서버를 사용하는 방식
  + DB 서버
    + 2차원 데이터 형태인 row, colum으로 구성된 테이블형 데이터 저장

  + DB 스토리지
    + 파일형태로 이미지나 텍스트가 저장
    + 파일형태가 되면 모든 데이터를 저장할 수 있다.

+ Sync방식으로 동기화한다.
+ 장점
  + 모든 노드에서 읽기,쓰기가 가능하고 모든 노드에서 데이터를 일관성있게 제공할 수 있다.
  + Fail over를 통해 고가용성을 제공한다.
    + 하나의 서버에 문제가 발생했을때, 서버 자체가 중단되는 SPOF(Single Point Of Failure)를 방지할 수 있다.
+ 단점
  + 여러 노드를 동기화하는 시간이 필요하기 때문에 Replication보다 쓰기 성능이 떨어진다.
  + 다른 노드에 중복을 요청함으로서 장애가 전파될 수 있다.
    + A노드에서 쓰기작업을 통해 복제를 요청했을때, B노드에서 장애로 인해 응답지연이 발생하게 되면 A노드도 응답을 받지 못하고 대기하게 된다.
      이때 C노드에서 복제 요청을 했을 때, A와B노드는 응답지연중이기 때문에 복제가 지연되어 장애가 전파될 수 있다.

  + 모든 노드에 중복 요청과정이 발생하기 때문에 동기화 시간이 길어지기 때문에 스케일링의 한계가 발생한다.
+ Galera 방식
  1. A노드에 쓰기작업으로 인해 트랜잭션이 발생하고 커밋이 발생한다.
  2. 디스크에 적용하기 전에 다른 노드에 복제를 요청한다.
  3. 다른 노드에서 복제 요청을 수락받으면 복제후 디스크에 작성한다.
+ Active-Active 방식
...
+ SLAVE로 데이터를 동기화할때 비동기화 작업으로 수행

+ 장점

  + DB요청의 60~80%가 읽기 작업이기 때문에 Replication으로 성능을 높일 수 있다.
  + 복제가 비동기 작업이기 때문에 지연시간이 발생하지 않는다.

+ 단점

  + 복제가 비동기 작업이기 때문에 일관성 있는 데이터를 얻지 못할 수도 있다.
  + Master 노드가 다운되면 복구 및 대처가 까다롭다.

+ 주의사항

  + Replication을 사용할때 MySQL버전이 다르다면, Slave 서버의 버전이 더 높아야한다.
  + Replication 가동시 Master, Slave 순으로 가동시켜야한다.

+ 처리방식

  1. Master노드가 쓰기 작업을 수행

  2. Master노드는 데이터를 저장하고 트랜잭션에 대한 로그(binary log)를 BINLOG에 저장한다.

  3. Slave노드의 I/O Thread는 Master노드의 BINLOG를 Replay Log에 저장한다.

  4. Slave노드의 SQL Thread는 Replay Log에 저장된 정보를 통해 데이터를 저장한다.

+ BINLOG 저장 방법

  + statement : 트랜잭션에서 수행한 쿼리를 그대로 저장하는 방법
  + row : 쿼리 작업의 before/after의 image를 저장해놓는 방법
