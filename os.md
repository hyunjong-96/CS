...
- Associatvie Cache (연관 캐시)
  - CPU가 메모리 주소를 요청했을때 서로 다른 주소지만 동일한 해시값으로 충돌할 수 있다. (충돌시 캐시를 변경해주는 방법이 있지만 비효율적이다) 이를 해결하기 위해 해시가 가리키는 블럭이 여러개로 설정이 가능하다.
  - Direct Mapped
    - 해시가 가리키는 공간이 1개인 경우, latency는 빠를수 있지만 충돌이 자주 발생한다.

  - Fully Associative
    - 모든 공간을 가리키는 경우, latency는 느리지만 충돌이 발생하지 않는다.

  - Set Associative
    - 2개 이상의 공간을 가리키는 경우, 두 캐시 방법이 극단적이기 때문에 해당 캐시를 자주 사용한다.
