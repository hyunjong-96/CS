...
- 부모 프로세스는 자식 프로세스의 종료 상태를 회수해야합니다. 하지만 자식 프로세스가 임의로 먼저 삭제되는 경우가 있는데, 부모 프로세스는 자식 프로세스의 종료 상태를 회수하지 못하여 `좀비 프로세스`가 됩니다.
  - 자식 프로세스가 먼저 종료되어도 부모 프로세스는 자식 프로세스의 최소한의 정보인 프로세스id와 종료 정보를 가지고 있게됩니다. 그리고 부모 프로세스가 좀비 프로세스의 종료 상태를 회수할때(wait()) 좀비 프로세스는 삭제가 되게 됩니다.
  - 좀비 프로세스가 많아지면 리소스의 유출이 발생할 수 있기 때문에 부모 프로세스에서 좀비 프로세스의 상태를 회수하여 종료시킵니다.
...
- 백그라운드 프로세스 중 PPID(부모프로세스)가 1인 프로세스로써, 사용자와 직접 소통하지 않고 백그라운드에서 작업을 수행하는 프로세스
...
- Stack
  - 장점
    - 컴파일 시점에 크기가 결정되기 때문에 메모리의 낭비가 적다(단편화 x)
...
- 페이징
  ...
  - 페이지 주소 변경 과정
    ...
    - MMU는 CPU 코어안에 탑재, Page Table은 (물리 메모리)RAM에 존재
      1. CPU 논리주소 요청
      2. TLB 히트 여부 확인
         1. TLB miss시, 물리 메모리에있는 PageTable에 접근하여 논리주소와 매핑된 물리주소를 가져온다
            1. PageTable에 매핑된 물리주소가 없다면 MMU가 page table갱신하여 매핑 저장
