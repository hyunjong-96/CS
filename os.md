...
- 캐시 : 레스터와 주 기억장치의 속도차이를 해결해주기 위한 저장소
...
- 가상메모리에 많이 사용되며 가상 메모리는 페이징으로 관리된다.
- 페이지 부재(page fault) 발생시 mmu(memory management unit)에 의해서 인터럽트가 발생시켜 프로세스를 wait상태로 변경 한 후, 요구하는 페이지를 메모리에 올리고 다시 실행한다.
...
- 종류
  ...
  - LRU (Least Recently Use) : 최근에 가장 오랫동안 사용하지 않은 페이지가 교체된다.
  - LFU (Least Frequently Use) : 최근에 사용빈도가 가장 적은 페이지가 교체된다.
  - OPT (Optimal) : 최근에 가장 사용되지 않을 페이지가 교체된다.
...
- 중기 스케줄러 : CPU에 할당되기 위해 너무 많은 프로세스가 몰리게 되면 메모리를 관리하기 위해 프로세스를 Swap영역으로 보내는 스케줄러
  - Swap out : swap영역으로 보냄
  - swap in : 메모리로 다시 들임
...
- CPU는 여러 프로세스가 있을때 스케줄링을 통해 프로세스를 관리한다.
...
- 프로세스 동기화 : 하나의 자원을 한순간에 하나의 프로세스만이 접근 가능하도록 제어하는 것
- 스레드 동기화 : 하나의 코드블럭 또는 메소드를 한 순간에 하나의 스레드만이 접근 가능하도록 제어하는 것
- 동기화 방법 : 뮤텍스, 세마포어
...
- 빠른 속도의 레지스터와 비교적 느린 주기억장치 간의 속도차이를 해결하기 위한 저장소
- CPU와 주 기억장치 사이에 존재하며, CPU가 주기억장치에 접근하는 횟수를 줄여 처리속도를 빠르게 도와준다.
